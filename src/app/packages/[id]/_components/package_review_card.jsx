...
const PackageReviewCard = ({
    ...
    const {
        mutate: NotHelpFullMutate,
        isPending: isNotHelpfullMutationLoading,
    } = useMutation({
        mutationKey: ["comments"], // Key for identifying the mutation
        mutationFn: () =>
            fetch(
            ...
            <div className="flex justify-between items-center px-4">
                ...
                {createdAt && (
                    <p className="text-tourHub-title2">
                        {moment(createdAt).format("ll")}creation date
                    </p>
                )}
            </div>
            ...
            <div className="text-tourHub-title2 font-normal text-14px leading-28px font-inter flex items-center gap-x-6 pl-[60px] pt-2">
                {isHelpFullOpenButton && (
                    <button
                        ...
                        disabled={
                            isHelpFullMutationLoading ||
                            !isHelpFullOpenButton ||
                            !clerkId // Disable button if loading or already clicked
                        }
                ...
                {isNotHelp && (
                    <button
                        className="hover:font-medium transition-all duration-300 disabled:text-gray-400"
                        disabled={
                            isNotHelpfullMutationLoading ||
                            !isNotHelp ||
                            !clerkId
                        } // Disable button if loading or already clicked
